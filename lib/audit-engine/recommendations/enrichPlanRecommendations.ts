...
export const enrichPlanRecommendations = (
  data: AuditFormValues,
  ...
  return findings.map((finding) => {
    if (finding.type !== 'OVERPROVISIONED_PLAN') return finding;
    ...
    const alternatives = computeAlternatives(tool, data);
    ...
    return {
      ...finding,
      title: `Downgrade ${tool.toolId} plan to ${bestAlternative.name}`,
      description: `Your ${tool.toolId} plan is overprovisioned for a ${teamScale}-scale team. Downgrading to ${bestAlternative.name} could save approximately $${bestAlternative.estimatedSavings}/mo.`,
      recommendation: `Switch to ${bestAlternative.name} to better match your team scale and reduce overhead spend.`,
      estimatedSavings: Math.max(finding.estimatedSavings, bestAlternative.estimatedSavings),
      alternatives,
    };
  });
};
